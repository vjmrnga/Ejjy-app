...
import { useFailedTransfers } from '../../../screens/Admin/hooks/useFailedTransfers';
...
export const Sidebar = () => {
	...
	const [failedTransfersCount, setFailedTransfersCount] = useState(0);
	...
	const failedTransfersRef = useRef(null);

	// METHODS
	useEffect(() => {
		if (user.user_type === userTypes.ADMIN) {
			getPendingTransactionsCount();
			pendingTransactionsRef.current = setInterval(() => {
				getPendingTransactionsCount();
			}, 5000);
		}

		return () => {
			clearInterval(pendingTransactionsRef.current);
		};
	}, []);

	useEffect(() => {
		if (user.user_type === userTypes.ADMIN) {
			const fetchFailedTransfer = () => {
				branches.forEach(({ id, name }) => {
					getFailedTansferCount({ branchId: id, branchName: name });
				});
			};

			if (failedTransfersRef.current) {
				clearInterval(failedTransfersRef.current);
			}

			fetchFailedTransfer();
			failedTransfersRef.current = setInterval(() => {
				fetchFailedTransfer();
			}, 5000);
		}
	...
	const getSidebarItems = useCallback(
		...
		[failedTransfersCount, pendingTransactionsCount],
	);
	...
	return (
		...
		>
			...
			<div className="Sidebar_sidebarList">
				{getSidebarItems()
					.filter((item) => item.userTypes.includes(user.user_type))
					.map((item) => (
